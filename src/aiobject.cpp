...
void DynamicObj::move(Uint32 dt) {
    ...
    } else // asteroids should rotate around their center of weight, as we must not change theta/gamma, we do this here
    {
        ...
        rot->setAngles((short) (90 + gamma + ttl * rot1 / timestep - 180), (short) theta + ttl * rot2 / timestep + 180,
                       (short) -phi);
    }
...
bool AIObj::selectMissileAir(AIObj **missile) {
    ...
    if (haveMissile(MISSILE_AIR3)) {
        ...
        missiletype = MISSILE_AIR1 - MISSILE1;
        sel = true;
    }
    return sel;
}
...
void AIObj::targetNearestGroundEnemy(AIObj **f) {
    ...
    if (target)
        if (distance(target) > 400) { target = NULL; }
}
...
void AIObj::aiAction(Uint32 dt, AIObj **f, AIObj **m, DynamicObj **c, DynamicObj **flare, DynamicObj **chaff) {
    ...
    if ((id >= MISSILE1 && id < MISSILE_MINE1) || (id >= FIGHTER1 && id <= FIGHTER2)) // missile or fighter
    {
        ...
        for (i = 0; i < (int) fg; i++) {
            ...
            smoke->setSmoke(tl->x - sx - forcex * fac, tl->y - sy - forcey * fac, tl->z - sz - forcez * fac, (int) phi,
                            39 - i);
        }
...
*/
                ...
                if (m[i]->ttl > 0)
                    if (m[i]->target == this) {
                        if (m[i]->id >= 0 && m[i]->id <= MISSILE_AIR3) {
                            if ((easymodel == 1 && fabs(theta) >= 30) ||
                                (easymodel == 2 && fplayer->elevatoreffect >= 0.5)) {
                                fireFlare(flare, m);
    ...
    if (ttl <= 0 && id >= MISSILE1 && id <= MISSILE2 && id != MISSILE_MINE1) {
        ...
        if (target != NULL &&
            ((id >= MISSILE1 && id <= MISSILE2) || (id >= FIGHTER1 && id <= FIGHTER2 && manoeverheight <= 0))) // is AGM
        {
            float dgamma = 0;
    ...
    if ((id >= FIGHTER1 && id <= FIGHTER2) || (id >= MISSILE1 && id <= MISSILE2) || (id >= FLAK1 && id <= FLAK2) ||
        (id >= TANK1 && id <= TANK2)) {
...
//    this->aw = a;
    ...
    if (id >= FIGHTER1 && id < FIGHTER_TRANSPORT) // for fighters do the following
    {
        if (!acttype && disttarget <= 1000 && manoevertheta <= 0) // no special action, near distance, no roll manoever
        {
            if (aw > 0) // positive angle
            {
                ...
                } else if (aw > 160.0F + 0.05 * intelligence &&
                           disttarget < 4 + 0.01 * intelligence) // target very near at the back
                {
                    manoeverstate = 1;
                    display("Manoever: Immelmann", LOG_ALL);
                } else if (aw > 160 && disttarget < 25) // target is at the back
                {
                    if (fabs(tl->y - myheight) > 7 && gamma >= 175 + intelligence / 100) // high enough over ground
                    {
                        ...
                        if (manoevertheta <= 0) {
                            manoevertheta =
                                    timestep * (100 + myrandom((400 - intelligence) / 8)); // turn hard left or right
    ...
    } else if (id >= TANK1 && id <= TANK2) // tanks
    {
        ...
        if (firecannonttl <= 0) {
            ...
            if (id == TANK_GROUND1)
                if (fabs(rectheta - theta) < 2 && fabs(aw) < 20 && disttarget < 35 && target->tl->y <= tl->y + 1 &&
                    target->tl->y >= tl->y - 1) {
                    fireCannon(c);
                    firecannonttl += firerate * timestep;
                }
